...
The normal Windows install runs the agent hidden in the background, shows a Dhruvanta-branded tray icon, and creates Desktop/Start Menu shortcuts. Use the tray menu for refresh, restart, stop, and update actions. Update checks open a visible Dhruvanta progress window; when a newer release exists, `Check for Updates` lets the owner download and install from that same window, while `Install Latest Update` starts the visible download/install flow immediately. The window shows percentage progress for release downloads and offers `Reinstall latest` for support-directed repair installs when the machine is already current. Use the Start Menu uninstall shortcut when removing the agent; it asks whether to keep or remove local data. A visible terminal window means the diagnostic console runner was used; close it and start the agent from the shortcut or tray.
