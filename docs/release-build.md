...
The setup executable installs hidden startup and Dhruvanta-branded shortcuts by default. Runtime state lives in `%LOCALAPPDATA%\Dhruvanta Systems\PrintAnywhereAgent\data` so versioned program-folder updates preserve pairing, backend URL, printer sharing, and health history. Direct setup installs and extracted-bundle installs both stop the older local listener and tray controller before starting the refreshed version, so updates replace the running agent instead of leaving duplicate tray icons. Managed per-user installs also prune older `printanywhere-agent-v*` program folders and stale release zip files after the refreshed bundle is installed; the stable `data` folder is kept. The Start Menu also includes one uninstall shortcut that asks whether to keep or remove local data.
...
must be signed with an OV/EV code-signing certificate issued to Dhruvanta
Systems before broader distribution so Windows can validate publisher identity
and SmartScreen can build reputation. A self-signed certificate is acceptable
only for internal lab testing because customer machines will not trust it by
default. See `docs/code-signing.md` for the release env vars and secret-file
layout.
