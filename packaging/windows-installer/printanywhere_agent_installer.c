...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE previous, PWSTR command_line, int show_command) {
  ...
  append_literal(script, BUFFER_CHARS * 2, L"$runtimeProcesses = Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object { $commandLine = [string]$_.CommandLine; $_.ProcessId -ne $PID -and -not [string]::IsNullOrWhiteSpace($commandLine) -and $commandLine -match \"PrintAnywhereAgent\" -and ($commandLine -match \"run-agent\\.ps1\" -or $commandLine -match \"dist[\\\\/]+index\\.js\" -or $commandLine -match \"node-win-x64[\\\\/]+node\\.exe\" -or $commandLine.StartsWith($installRoot, [System.StringComparison]::OrdinalIgnoreCase)) } | Select-Object -ExpandProperty ProcessId -Unique\r\n");
  append_literal(script, BUFFER_CHARS * 2, L"foreach ($processId in (($owners + $runtimeProcesses) | Select-Object -Unique)) { Stop-Process -Id $processId -Force -ErrorAction SilentlyContinue }\r\n");
  append_literal(script, BUFFER_CHARS * 2, L"for ($attempt = 0; $attempt -lt 10; $attempt += 1) { $listener = Get-NetTCPConnection -LocalPort 43100 -ErrorAction SilentlyContinue | Where-Object { $_.OwningProcess -gt 0 -and $_.State -eq \"Listen\" } | Select-Object -First 1; if (-not $listener) { break }; Start-Sleep -Milliseconds 500 }\r\n");
